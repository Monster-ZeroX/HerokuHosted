      ...
      <GlassCard title={torrent?.name || 'Loading...'} icon="🎞️" action={<Link className="button-ghost" href="/dashboard">Back</Link>}>
        <div style={{ display: 'grid', gap: 10 }}>
          <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
            <span className="badge">{torrent?.status || 'pending'}</span>
            <span style={{ color: 'var(--text-secondary)' }}>Progress {torrent?.progress?.toFixed?.(1) ?? 0}%</span>
          </div>
          <div className="progress-shell"><div className="progress-bar" style={{ width: `${torrent?.progress ?? 0}%` }} /></div>
      ...
      {torrent?.tmdb && (
        <GlassCard title="Movie intel" icon="🍿">
          <div style={{ display: 'flex', gap: 16 }}>
            ...
            <div>
              <div className="metric-value" style={{ fontSize: '1.4rem' }}>{torrent.tmdb.title}</div>
              <div className="metric-label">{torrent.tmdb.release_date}</div>
              <div className="badge success" style={{ marginTop: 8 }}>⭐ {torrent.tmdb.rating}</div>
              <p style={{ color: 'var(--text-secondary)' }}>{torrent.tmdb.overview}</p>
            </div>
          </div>
        </GlassCard>
      )}
